...
func TestSliceString(t *testing.T) {
	codec := lexy.SliceOf(aStringCodec)
...
func TestSlicePtrUint8(t *testing.T) {
	p := ptr[uint8]
	codec := lexy.SliceOf(lexy.PointerTo(uint8Codec))
...
func TestSlicePtrString(t *testing.T) {
	codec := lexy.SliceOf(lexy.PointerTo(aStringCodec))
	testCodec(t, codec, []testCase[[]*string]{
		{"nil", nil, []byte{pNilFirst}},
		{"empty", []*string{}, []byte{pNonNil}},
...
func TestSliceSliceInt32(t *testing.T) {
	codec := lexy.SliceOf(lexy.SliceOf(int32Codec))
	testCodec(t, codec, []testCase[[][]int32]{
		{"nil", nil, []byte{pNilFirst}},
		{"[]", [][]int32{}, []byte{pNonNil}},
...
func TestSliceSliceString(t *testing.T) {
	codec := lexy.SliceOf(lexy.SliceOf(aStringCodec))
...
func TestSliceNilsLast(t *testing.T) {
	encodeFirst := encoderFor(lexy.SliceOf(int32Codec))
	encodeLast := encoderFor(lexy.SliceOfNilsLast(int32Codec))
